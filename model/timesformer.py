...
class TimeSformer(nn.Module):
    def __init__(
        ...
        num_positions = frames * num_patches
        ...
        for _ in range(depth):
            ...
            time_attn = Attention(dim, dim_head = self.dim_head, heads = heads, dropout = attn_dropout)
            spatial_attn = Attention(dim, dim_head = self.dim_head, heads = heads, dropout = attn_dropout)

            if shift_tokens:
                time_attn, spatial_attn, ff = map(lambda t: PreTokenShift(frames, t), (time_attn, spatial_attn, ff))
        ...
        self.classify_head = nn.Sequential(
            nn.LayerNorm(dim),
            nn.Linear(dim, number_classes)
        )

    def forward(self, video, mask = None):
        # 调整输入维度: [B,C,T,H,W] -> [B,T,C,H,W]
        ...
        if self._endpoint == 'feature':
            return cls_token
        logits = self.classify_head(cls_token)
...
if __name__ == '__main__':
    ...
    model = TimeSformer(
        dim=512,  # 特征维度
        frames=64,  # 视频帧数
        number_classes=4,  # 分类类别数
        image_size=224,  # 图像尺寸
        patch_size=16,  # 分块大小
        input_channels=3,  # 输入通道（RGB）
        depth=12,  # Transformer 层数
        heads=8,  # 注意力头数
        attn_dropout=0.5,
        ff_dropout=0.5,
        rotary_emb=True,  # 使用旋转位置编码
        shift_tokens=True,  # 是否使用token shift
        endpoint = 'prediction' #端点 feature/logits/prediction
    ).to(device)

    # (batch, channel, time, height, width)
    x = torch.randn(2, 3, 64, 224, 224).to(device)
    with torch.no_grad():
        outputs = model(x)
    print(outputs.shape)
