...
from utils.flownet.submodules import *

'Parameter count = 45,371,666'


class FlowNetSD(nn.Module):
    def __init__(self, input_channels=6, batchNorm=True, training=True):
...
if __name__ == '__main__':
    ...
    model = FlowNetSD(input_channels=6, batchNorm=True, training=False).to(device)

    # 使用能被 64 整除的尺寸（如 384x512）
    x = torch.randn(2, 6, 384, 512).to(device)

    with torch.no_grad():
        outputs = model(x)

    if isinstance(outputs, tuple):  # 训练模式返回多个输出
        for i, out in enumerate(outputs):
            print(f"flow{i + 2} shape:", out.shape)
    else:  # 测试模式只返回 flow2
        print("flow2 shape:", outputs.shape)
